...
export function makeRng(seed = 1, state) {
  // `state` (optional) resumes an existing stream: pass a value previously read
  // from rng.state() and the returned generator continues the SAME sequence
  // instead of restarting from `seed`. Used by the roguelike run-persistence
  // (Phase E) so a reload mid-run is deterministic. Additive — callers that
  // pass only a seed, and ignore rng.state, are completely unaffected.
  let a = (state === undefined ? seed : state) >>> 0;
  const rng = function () {
    a |= 0;
    a = (a + 0x6d2b79f5) | 0;
  ...
  rng.state = () => a >>> 0; // current internal state, for persistence
  return rng;
}
